...
class Helper(object):
	...
	@classmethod
	def get_batch(cls, batch, word_vec):
	...
	@classmethod
	def get_word_dict(cls, sentences, tokenize=True):
	...
	@classmethod
	def get_glove(cls, word_dict, glove_path):
	...
	@classmethod
	def get_skipthought(cls, word_dict, glove_path):
		...
		print('Found {0}(/{1}) words with skipthought vectors'.format(
			len(word_vec), len(word_dict)))
	...
	@classmethod
	def build_vocab(cls, sentences, glove_path, skipthought = False, tokenize=False):
		word_dict = cls.get_word_dict(sentences, tokenize)
		if skipthought:
			word_vec = cls.get_skipthought(word_dict, glove_path)
		else:
			word_vec = cls.get_glove(word_dict, glove_path)
	...
	@classmethod
	def txt_to_sent(cls, sentences, word_vec, tokenize=False):

		sentences = [['<s>']+s.split()+['</s>'] if not tokenize else 
			['<s>']+nltk.word_tokenize(s)+['</s>'] for s in sentences]
